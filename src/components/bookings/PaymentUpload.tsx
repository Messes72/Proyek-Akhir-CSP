...
export default function PaymentUpload({ bookingId, onUploadComplete }: PaymentUploadProps) {
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (updateError) {
        console.error('Supabase Database Error Details:', JSON.stringify(updateError, null, 2));
        throw updateError;
      }
