...
export default function BookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (error) {
      ...
      if (data.proof_of_payment_url) {
          const { data: signedData } = await supabase.storage
              .from('payment-proofs')
              .createSignedUrl(data.proof_of_payment_url, 3600); // 1 hour
  ...
  if (!booking) return <div className="p-12 text-center text-red-500">Booking tidak ditemukan.</div>;
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-6">
            <Link href="/bookings/my" className="text-green-600 hover:text-green-800 font-medium"> 
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                ID: {booking.id} <button onClick={handleCopyId} className="text-blue-500 ml-2" title="Copy ID">📋</button>
              </p>
            </div>
          ...
          <div className="border-t border-gray-200">
            <dl>
              ...
              {/* Timing */}
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm font-medium text-gray-500">Waktu Main</dt>
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                    <div>{new Date(booking.start_time).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
                    ...
                    {booking.proof_of_payment_url ? (
                         <div className="mt-2">
                             {imageSignedUrl ? (
                                 ...
                                 <span className="text-gray-400 italic">Memuat gambar...</span>
                             )}
                             <p className="text-xs text-gray-500 mt-1">Uploaded at: {new Date(booking.created_at).toLocaleString('id-ID')}</p> 
                             
                             {booking.status === 'pending' && (
                                <div className="mt-4 border-t pt-2">
                                    <p className="text-xs text-gray-500 mb-2">Ingin mengganti bukti bayar?</p>
                                    <PaymentUpload bookingId={booking.id} onUploadComplete={fetchBooking} />
                                </div>
                             )}
                         </div>
                    ) : (
